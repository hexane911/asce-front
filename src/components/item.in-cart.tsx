...
const CartItem = ({
  ...
  const inCart = cart.find((el) => el.id === id)?.quantity || 0;
  ...
  return (
    <div
      className={classNames(classname, { disabled, inOrder })}
      style={{ animationDelay: `${animationDelay || 0}ms` }}
      onAnimationEnd={() => setClassname("cart-item")}
    >
      <div
        ...
        >
          ...
          <div className="in-cart cart-item__in-cart">
            {!inOrder && (
              <>
                <div
                  className={classNames("in-cart__quan plus", {disabled: product.in_stock_amount <= inCart})}
